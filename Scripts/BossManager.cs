       ...
       /*  for(int i = 0; i < shotsToFire.Length; i++)
        {
            ...
            if(shotsToFire[i].shotCounter <= 0)
            {
                ...
                Instantiate(shotsToFire[i].shot, shotsToFire[i].firePoint.position, shotsToFire[i].firePoint.rotation);
            }
        } */
        if(!battleEnding)
        {
            ...
            else
            {
                for(int i = 0; i < phases[currentPhase].phaseShots.Length; i++)
                {
                    ...
                    if(phases[currentPhase].phaseShots[i].shotCounter <= 0)
                    {
                        ...
                        Instantiate(phases[currentPhase].phaseShots[i].shot, phases[currentPhase].phaseShots[i].firePoint.position, phases[currentPhase].phaseShots[i].firePoint.rotation);
                    }
                }
            }
        }
